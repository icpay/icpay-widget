...
export class ICPayPayButton extends LitElement {
  ...
  private getSdk(): WidgetSdk {
    if (!this.sdk) {
      ...
      const wid = (this.lastWalletId || '').toLowerCase();
      if (wid) cfg = { ...cfg, icWalletAdapterId: wid };
      ...
      try {
        ...
        if (sel?.x402Accepts) {
          try {
            ...
            debugLog(this.config?.debug || false, 'X402 payment failed (IC/SOL selection), falling back', {
              message: x402Err?.message, code: x402Err?.code, data: x402Err?.details || x402Err?.data
            });
