...
export class ICPayPremiumContent extends LitElement {
  ...
  private async createOnrampIntent() {
    try {
      ...
      const resp = await (sdk as any).startOnrampUsd(this.config.priceUsd, undefined, { context: 'premium:onramp', onrampPayment: true, onrampProvider: 'coinbase' });
      const url =
  ...
  private onSelectBalanceSymbol = async (shortcode: string) => {
        ...
        try {
          ...
          if (sel?.x402Accepts) {
            try {
              ...
              // Surface x402 error but allow fallback
              handleWidgetError(e, {
                onError: (error) => {
                  this.dispatchEvent(new CustomEvent('icpay-error', { detail: error, bubbles: true }));
                  if (shouldShowErrorToUser(error)) {
    ...
    if (action === 'pay') {
      // IC/SOL flow: prefer x402 for Solana tokens when available, otherwise fallback to normal flow
      try {
